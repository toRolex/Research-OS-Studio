...
在开始拟大纲前，用用户原始材料清单对照 profile：ablation table、scaling curve、failure-case analysis、proof block、related-work synthesis 等。即使用户没有材料，也输出真实缺口，不把空项目视为无需检查。
...
**完成条件**：每个拟采用的槽位有真实材料或明确缺口，拟定的大纲不暗示未完成内容已经存在。
...
3. **原始材料读取范围**：检查研究笔记、草稿等是否混入 style 来源、原文或摘要。可分离时，提供原文件路径及准确行/页范围，让评审者直接读取全部研究证据及必要上下文，仅排除 style 内容；不以执行者摘要替代原始证据，不让评审者整文件读取混合材料。若证据与 style 内容不可分离，或宿主不能按范围读取，暂停独立评审，标“未独立评审”及隔离阻碍，请用户提供分离材料；强制评审时停于暂定稿。
