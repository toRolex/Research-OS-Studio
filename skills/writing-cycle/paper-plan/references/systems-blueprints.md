> Planning-only adaptation of Orchestra systems-paper-writing/references/section-blueprints.md. Full paragraph methods retained under LICENSE-Orchestra.txt. Read for systems research. Treat the following as unfilled outline worksheets, not manuscript text or execution commands: map each slot to existing materials and mark absent slots DATA_NEEDED. All lengths are illustrative; hypotheses, comparative wins, LOC, deployment, public release, causal explanations and production numbers require actual sources. A design alternative without a comparison is rationale, not measured evidence. Use the existing code/prototype as a read-only source; no implementation or new evaluation is performed.

# Section-by-Section Blueprints for Systems Papers

Detailed paragraph-level templates for each section of a 10–12 page systems paper. The paragraph worksheets are retained; paper-specific exemplar/award assertions are omitted.
...
S4 — Results: What are the headline numbers?
     (e.g., "Evaluation on [actual testbed] shows [supported result, including failures] over [actual baselines]")
...
supporting [only the scope established by the actual evidence; describe failures or inconclusive results where applicable].
```
