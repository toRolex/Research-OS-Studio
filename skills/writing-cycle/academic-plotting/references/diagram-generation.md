...
Use these six sections as an editable design specification for SVG, DOT, Mermaid or TikZ with the user's existing tools. AI rendering is optional and requires explicit authorization for the provider, uploaded content, cost and attempt limit. A saved prompt does not reproduce stochastic pixels and a PNG is not an editable diagram: preserve the component/edge specification and editable vector/text source too. If unavailable, deliver a clearly labeled unrendered draft and stop.

Every diagram carries a visible “Schematic — not experimental evidence” label and a corresponding caption. In mixed figures mark each panel separately. All labels, quantities and arrows come from supplied context; uncertain relationships remain marked proposed/unknown. Do not invent qualitative model outputs, screenshots or measured results.
...
This is the MOST important section. Copy the chosen full style block from [diagram-styles.md](diagram-styles.md) and expand with more detail. Be extremely specific about visual characteristics.
...
- Include the schematic disclosure; keep the descriptive caption in its companion text
...
Only generate the authorized number of attempts. First check every label, node, edge and its direction against the specification: a semantic error disqualifies an attempt regardless of aesthetics. Then use the following optional style rubric among semantically correct attempts; scores do not certify scientific validity.
...
Use when the diagram is simple enough for deterministic output and TikZ plus its `arrows.meta` library already exist. The excerpt is a picture body, not a standalone TeX document:
...
Sketch the logical flow before rendering with the authorized tool:
...
Validate the structure against source context, then produce the six-section design specification. These examples illustrate invented systems, not the user's architecture; replace all entities and numerical labels from real context.
