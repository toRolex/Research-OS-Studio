...
Adapted from Orchestra creative-thinking-for-research. All eight framework workflows, examples, self-checks, combination protocol and creative-block guide are retained. Examples and cognitive-science attributions are upstream heuristics, not verified evidence for the user's research. Check source-domain facts before using them as premises.

## Index

- [F1 Bisociation](#framework-1-combinatorial-creativity-bisociation) — primitives and cross-product matrix
- [F2 Representational change](#framework-2-problem-reformulation-representational-change) — hidden assumptions and alternative formulations
- [F3 Structure-mapping](#framework-3-analogical-reasoning-structure-mapping) — relational mechanisms and predictions
- [F4 Boden](#framework-4-constraint-manipulation-bodens-framework) — hard/soft/hidden constraints
- [F5 Negation/TRIZ](#framework-5-negation-and-inversion) — inversion and engineering principles
- [F6 Polya](#framework-6-abstraction-and-generalization-laddering) — generalize/specialize/analogize
- [F7 Adjacent possible](#framework-7-the-adjacent-possible-kauffman--johnson) — evidence-backed enablers
- [F8 Janusian](#framework-8-janusian-and-dialectical-thinking) — synthesis without premature compromise
- [Full protocol](#combining-frameworks-a-creative-thinking-protocol) — only within an explicitly authorized eight-framework session
- [Creative blocks](#common-creative-blocks-and-unblocking-strategies) — selection guide

**Execution boundary:** the main Skill's original question, read/write authorization and budget govern every framework. Testing/building/validating below means design a discriminating test, not run it. Soft conceptual constraints may be questioned; user permission, privacy/ethics and known physical/logical constraints remain hard. Preserve questions and uncertainty rather than accepting or killing ideas by taste.
...
3. **Evaluate each negation**:
   - Incoherent → retain the attempted route with the precise inconsistency; do not present it as a supported candidate
   - Already explored → list the assumptions behind the earlier attempt and check, against the supplied materials, whether compute, scale, data, tools or conditions have changed; annotate rather than discard
...
**Illustrative enablers (verify availability in the supplied materials; not current capability claims)**:

| Enabler | Newly Possible |
|---------|---------------|
| Longer context windows available to the project | Full-codebase reasoning, book-length analysis |
| Lower inference costs within an observed budget | Real-time agentic loops, always-on AI assistants |
| Open-weight models adequate for the target task | Reproducible research on frontier capabilities |
...
**Timing Signal**: Record evidence for each enabling condition and distinguish confirmed availability from speculation. If a required capability is confirmed unavailable under the user's hard constraints, retain the idea as a counterfactual with that constraint and a reopening condition; uncertain availability stays an annotation. Age alone establishes neither prior work nor novelty. List related-work questions for a separately authorized search rather than searching or excluding ideas here.
...
|--------------|------------|--------|
| Consistency AND Availability (distributed systems) | Formalize the partition trade-off; investigate which operations require coordination (Raft and CRDTs do not remove CAP limits) | Foundation of distributed systems theory |
...
| Compression AND Quality | Learned codecs explore rate–distortion–perception trade-offs under explicit source models; they do not violate information-theoretic limits | Redefined compression research |
...
Apply the two-sentence test locally (a clarity check, not independent review):
> "**[Domain] currently struggles with [problem] because [reason].** We propose [approach] by [mechanism], hypothesizing [insight]."

Preserve ideas and named risks after all four phases; passing the two-sentence test establishes only a clear candidate, not novelty, correctness or a decision to pursue it.
